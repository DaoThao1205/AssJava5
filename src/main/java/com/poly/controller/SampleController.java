...
public class SampleController {
    ...
    private String login(HttpSession session, @RequestParam("tenTaiKhoan") String user, @RequestParam("matKhau") String password,
                         ModelMap model) {
        if(user.length()==0){
            model.put("errorLogin", "Vui Lòng Nhập Tên Đăng Nhập!");
            return  "index.html";
        }
        if(password.length()==0){
            model.put("errorLogin", "Vui Lòng Nhập Mật Khẩu!");
            return  "index.html";
        }
        if(user.length()<1||user.length()>50){
            model.put("errorLogin", "Tên Đăng Nhập Phải TỪ 1-50 Ký Tự!");
            return  "index.html";
        }
        if(password.length()<6||password.length()>20){
            model.put("errorLogin", "Mật Khẩu Phải TỪ 6-20 Ký Tự!");
            return  "index.html";
        }
